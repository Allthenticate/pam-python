    ...
    PyObject* self, PyObject* args, PyObject* kwds)
{
  ...
  int			errnum;
#ifdef __APPLE__
  const int topErrNum = 30; // pam_constants.h
#else
  const int topErrNum = _PAM_RETURN_VALUES;
#endif
  PyObject*		result = 0;
  ...
  if (errnum >= debug_magic && errnum < debug_magic + topErrNum)
  {
